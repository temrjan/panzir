...
pub enum Error {
    /// Ошибка системного D-Bus / udisks2: служба ответила ошибкой, которую
    /// ядро не классифицирует, либо недоступна вовсе. Отказ в правах сюда
    /// НЕ входит — у него собственный вариант [`Error::NotAuthorized`], иначе
    /// «нельзя» неотличимо от «шина умерла». Конверсия из `zbus::Error` —
    /// ручная, в `udisks.rs`: там живут все имена ошибок udisks2.
    #[error("udisks2: {0}")]
    Udisks(#[source] zbus::Error),

    /// udisks2 ответил отказом polkit. Служба жива и ответила безупречно —
    /// она сказала «нельзя»; оттенок отказа — в [`AuthRefusal`].
    #[error("udisks2 refused authorization: {reason:?}")]
    NotAuthorized {
        /// Что именно ответил polkit.
        reason: AuthRefusal,
    },
    ...
    /// `systemd --user` не завёл или не снял таймер автозакрытия. Отдельный
    /// вариант, а не [`Error::Command`]: окно обязано сказать человеку, что
    /// без часов хранилище само не закроется, — это не «утилита упала».
    #[error("systemd --user: {cmd} ({status})")]
    Schedule {
        /// Командная строка без секретов.
        cmd: String,
        /// Код завершения или состояние, на котором сдались.
        status: String,
    },
...
/// Оттенок отказа polkit, пришедшего от udisks2. Имена D-Bus в этот тип не
/// попадают: окно про zbus не знает и знать не должно.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AuthRefusal {
    /// Политика ответила «нет» без запроса подтверждения — пароль не поможет.
    Denied,
    /// Политика требует подтверждения, а получить его в этом вызове нельзя:
    /// агента нет или вызов сам запретил диалог. Имя ошибки эти две причины
    /// не различает — и тип не различает.
    NeedsConfirmation,
    /// Человек закрыл диалог подтверждения сам.
    Dismissed,
}
